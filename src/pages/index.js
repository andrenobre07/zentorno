...
export default function Home() {
  ...
  // Mova a definição do array para fora das funções para que fique disponível para todo o componente.
  const featuredCars = [
    {
  ...
  const handleNext = () => {
    if (currentSlide < featuredCars.length - 1) {
      setCurrentSlide(currentSlide + 1);
    } else {
      setCurrentSlide(0); // Volta ao início quando chega ao fim
    }
  };
  ...
  const handlePrev = () => {
    if (currentSlide > 0) {
      ...
      setCurrentSlide(featuredCars.length - 1); // Vai para o último quando está no início
    }
  };
  ...
  return (
    <main className="bg-black text-white">
      ...
      {/* Hero section e demais seções do site */}
      ...
      <section id="carros-destaque" className="py-20 bg-gradient-to-b from-black to-gray-900">
        <div className="container mx-auto px-4">
          ...
          {/* ABAIXO: ÚNICA MUDANÇA. Adicionamos <div className="flex justify-center"> em volta */}
          <div className="flex justify-center">
            <div className="relative overflow-hidden max-w-4xl w-full mx-auto" 
              ...
              <button 
                onClick={handlePrev}
                className="absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-black/50 hover:bg-blue-600/70 text-white w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm transition-all duration-300 border border-white/10 shadow-lg md:left-4 -left-2"
              ...
              <button 
                onClick={handleNext}
                className="absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-black/50 hover:bg-blue-600/70 text-white w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm transition-all duration-300 border border-white/10 shadow-lg md:right-4 -right-2"
                  ...
                  <div key={index} className="w-full flex-shrink-0 px-4">
                    <div className="flex flex-col md:flex-row items-center">
                      <div className="w-full md:w-1/2 relative rounded-lg overflow-hidden shadow-2xl mb-8 md:mb-0 group">
                        ...
                        <div className="absolute bottom-4 left-4 bg-blue-600/80 text-white px-3 py-1 rounded-full text-sm backdrop-blur-sm">
                          {car.category}
                        </div>
                      </div>
                      <div className="w-full md:w-1/2 md:pl-12 flex flex-col items-center md:items-start">
                        ...
                        <div className="space-y-4 w-full max-w-sm mb-8">
                          <div className="flex justify-between">
                            ...
                            <span className="font-bold">{car.acceleration}</span>
                          </div>
                          <div className="w-full bg-gray-700 h-1 rounded-full overflow-hidden">
                            <div className="bg-blue-500 h-1 rounded-full" style={{ width: car.accelerationScore }}></div>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Velocidade Máxima</span>
                            <span className="font-bold">{car.topSpeed}</span>
                          </div>
                          <div className="w-full bg-gray-700 h-1 rounded-full overflow-hidden">
                            <div className="bg-blue-500 h-1 rounded-full" style={{ width: car.speedScore }}></div>
                          </div>
                        </div>
                        <Link href={car.link}>
                          <button className="px-8 py-3 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-blue-600/30">
                            Personalizar Agora
                          </button>
                        </Link>
                      </div>
                ...
                {featuredCars.map((_, index) => (
                  <button 
                    ...
                    className={`h-3 rounded-full transition-all duration-300 ${
                      currentSlide === index ? 'bg-blue-500 w-8' : 'bg-gray-600 w-3'
                    }`}
